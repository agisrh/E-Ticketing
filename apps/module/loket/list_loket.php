    ...
    <!--------------button action--------------->
              <div class="col-md-8 pull-left">
                  <div class="compose-btn">
                       <a class="btn btn-sm btn-primary" href="?module=add_loket"><i class="fa fa-plus"></i> Tambah Loket</a>
    ...
    <!------------------content table------------>
                              ...
                              <table id="employees" class="dataTables-example table table-striped responsive-utilities jambo_table bulk_action">
                                        <thead>
                                            <tr class="headings">
                                                ...
                                                <th class="column-title" width="30%">Nama Loket </th>
                            ...
                            <tbody>
                                ...
                                echo"<tr class='even pointer'>
                                    ...
                                    <td class=' last'>
                                    <a href='?module=edit_loket&id=$row[id_loket]' title='Edit' class='btn btn-sm btn-primary'><i class='fa fa-pencil'></i></a>
                                    <a href='?module=loket&act=hapus&id=$row[id_loket]' title='Hapus'  class='btn btn-sm btn-danger'><i class='fa fa-remove' onClick=\"return confirm('Apakah Anda benar-benar akan menghapus data ini ?')\"></i></a>
                                    </td>
                                </tr>";
                                }
                                ?>
...
if($_GET['act']=='hapus'){
    $id     = $_GET['id'];
    $query->delete_loket($id);
    $query->log_aktifitas($_SESSION[id_user],'Menghapus loket tiket',date('Y-m-d'),date('H:i:s'));
    echo "<script>window.location='?module=loket';</script>";
}